...
// StyleAgent performs LLM-powered code style analysis on PRs
...
// Execute runs the LLM-powered style analysis
func (a *StyleAgent) Execute(ctx context.Context, input types.AgentReviewInput) (*types.AgentResult, error) {
	info := activity.GetInfo(ctx)
	workflowID := info.WorkflowExecution.ID

	a.Logger.Info("Style agent started",
	...
	userPrompt := fmt.Sprintf(`Review this Pull Request for code style and quality:
...
Analyze the code changes and return your findings in JSON format as specified in the system prompt.`,
	...
	review, rawContent := parseStructuredReview(response.Content, "Style")
	if rawContent != "" {
	...
	a.Logger.Info("Style agent completed",
		zap.String("workflow_id", workflowID),
...
func (a *StyleAgent) formatFindings(review *StructuredReview, rawContent string) []string {
	if rawContent != "" {
		return []string{
	...
	if len(review.Findings) == 0 {
		findings = append(findings, "✓ No style issues found")
		return findings
	}
...
func (a *StyleAgent) mapStatus(status string) string {
	switch status {
	case "passed":
...
func (a *StyleAgent) severityEmoji(severity string) string {
	switch severity {
	case "critical":
		return "🚨"
	case "high":
...
func (a *StyleAgent) publishProgress(workflowID string, progress int) {
	a.EventBus.Publish(types.WorkflowEvent{
		WorkflowID: workflowID,
...
func (a *StyleAgent) handleError(workflowID string, err error) (*types.AgentResult, error) {
	a.Logger.Error("Style agent failed",
		zap.String("workflow_id", workflowID),
		zap.Error(err))
